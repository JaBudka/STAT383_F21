...
timey_adk_data <- na.omit(raw_adk_data) %>%
  mutate(format(strptime(incident_start_time, "%I:%M %p"), "%H:%M"))
  # timey_adk_data %>%
  #   mutate(true_adk_start_time = true_start_time)
  # mutate(true_start_date = paste(incident_start_date,"",incident_start_time)) %>%
  # mdy_hm(true_start_date)
```
