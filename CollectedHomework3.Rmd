...
```{r}
babyname_count <- select(babynames, year, sex, n) %>%
  group_by(year, sex) %>%
    summarise(n = sum(n))
...
ggplot() + 
    geom_line(bnc_F,mapping = aes(x = year, y = n, color = "red"))  +   
    geom_line(bnc_M, mapping = aes(x = year, y = n, color = "blue")) + 
    labs(
...
ggplot() + 
    geom_line(bnt_F,mapping = aes(x = year, y = numb, color = "red"))  +   
    geom_line(bnt_M, mapping = aes(x = year, y = numb, color = "blue")) + 
    labs(
  title = "Total number of different baby names of each sex versus year",
  x = "Birth Year",
  y = "Number of Baby Names") +
  theme(legend.title = element_text(colour="black", size=10, face="bold")) +
  scale_color_manual(labels = c("Female", "Male"), values = c("red", "blue"))
